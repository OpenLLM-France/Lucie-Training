...
BASE_OUTPUT_PATH=out/fr_leaderboard/lucie
...
run_evaluation() {
    ...
    if [ ! -d "${BASE_OUTPUT_PATH}/${output_subdir}" ]; then
        ...
        srun --exclusive --ntasks=1 lm-eval \
            --model vllm \
            --model_args "pretrained=${cp_path},tokenizer=${tokenizer_path},dtype=bfloat16${peft_args},add_bos_token=True,tensor_parallel_size=1,dtype=auto,gpu_memory_utilization=0.8,data_parallel_size=4" \
			${additional_args} \
